...
function wave_install_project_root(): string
{
    return dirname(__DIR__, 2);
}

function wave_install_os(): string
{
    $os = PHP_OS;
...
function wave_install_convert_slashes(string $path): string
{
    return str_replace('/', '\\', $path);
}

function wave_install_copy_env(string $projectRoot): void
{
...
function wave_install_redirect_home_if_needed(): void
{
    ...
    if ($path !== '/') {
        ...
        header('Location: '.$protocol.($_SERVER['HTTP_HOST'] ?? 'localhost'));
        exit;
    }
}

function wave_install_render_error(string $message, int $code = 500): never
{
...
function wave_install_first_executable(array $candidates): ?string
{
...
function wave_install_which(string $command): ?string
{
    $os = wave_install_os();
    $which = $os === 'Windows' ? 'where' : 'command -v';
...
function wave_install_resolve_composer_binary(string $projectRoot, string $phpPath): string
{
    ...
    foreach ($candidates as $candidate) {
        ...
        if ($os !== 'Windows' && ! is_executable($candidate)) {
            continue;
        }
    ...
    wave_install_render_error('Composer binary not found. Install Composer (Laravel Herd includes Composer) and try again.');
}

function wave_install_build_command(string $projectRoot, string $phpPath, string $composerPath, string $os): string
{
    ...
    return $os === 'Windows' ? wave_install_convert_slashes($command) : $command;
}

function wave_install_wave_logo_svg(): string
{
    return <<<'SVG'
...
function wave_install_base_styles(): string
{
    return <<<'CSS'
*,::after,::before{box-sizing:border-box}body{margin:0;font-family:ui-sans-serif,system-ui,sans-serif;line-height:1.5}.fixed{position:fixed}.relative{position:relative}.inset-0{inset:0}.top-0{top:0}.left-0{left:0}.bottom-0{bottom:0}.z-10{z-index:10}.z-20{z-index:20}.z-30{z-index:30}.flex{display:flex}.h-screen{height:100vh}.w-screen{width:100vw}.w-full{width:100%}.flex-col{flex-direction:column}.items-start{align-items:flex-start}.items-center{align-items:center}.justify-start{justify-content:flex-start}.justify-center{justify-content:center}.overflow-hidden{overflow:hidden}.overflow-y-scroll{overflow-y:auto}.rounded-xl{border-radius:.75rem}.bg-black{background:#000}.text-white{color:#fff}.text-red-500{color:#ef4444}.p-5{padding:1.25rem}.pt-16{padding-top:4rem}.pt-4{padding-top:1rem}.pb-3{padding-bottom:.75rem}.pl-5{padding-left:1.25rem}.font-mono{font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,monospace}.font-sans{font-family:ui-sans-serif,system-ui,sans-serif}.text-xs{font-size:.75rem;line-height:1rem}.font-bold{font-weight:700}.backdrop-blur-sm{backdrop-filter:blur(4px)}.bg-opacity-20{background:rgba(0,0,0,.2)}#container{scroll-behavior:smooth}
CSS;
}
