<nav class="flex justify-between items-center py-2 w-full text-xs">
    <ol role="list" class="flex items-center space-x-1">
        <li>
            <button wire:click="goToDirectory('/')" class="@if($this->isRootDirectory()){{ 'bg-gray-100 dark:bg-zinc-800' }}@else{{ 'bg-transparent' }}@endif inline-flex items-center px-3 py-2 font-normal text-center text-gray-900 dark:text-zinc-200 rounded-md hover:bg-gray-100 dark:hover:bg-zinc-800 focus:outline-none">
        ...
        @foreach($breadcrumbs as $breadcrumb)
            <li><svg class="flex-shrink-0 w-5 h-5 text-gray-300 dark:text-zinc-600" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true"><path d="M5.555 17.776l8-16 .894.448-8 16-.894-.448z"></path></svg></li>
            <li>
                <button 
                    wire:click="goToDirectory('{{ $breadcrumb->location }}')" 
                    class="@if($loop->last) bg-gray-100 dark:bg-zinc-800 @else bg-transparent @endif
                        inline-flex items-center px-3 py-2 font-normal text-center text-gray-900 dark:text-zinc-200 rounded-md hover:bg-gray-100 dark:hover:bg-zinc-800 focus:outline-none">
                    @if($loop->last)
    ...
    <div class="hidden items-center h-full lg:flex">
        <button @click="activeFileDrawer=!activeFileDrawer" class="p-2 h-full rounded-md hover:bg-gray-100 dark:hover:bg-zinc-800 text-gray-700 dark:text-zinc-300">
