<x-filament::modal id="move-file-modal" width="lg">
    <x-slot name="trigger">
        <button class="flex items-center px-4 py-2 text-xs font-medium text-gray-700 bg-white hover:bg-gray-100">
            ...
            <span class="hidden xl:inline">Mută</span>
        </button>
    </x-slot>
    ...
    <form wire:submit="moveFileOrFolder">
        <x-filament::input.wrapper>
            <x-filament::input.select
                ...
                label="Selectează folderul destinație"
                placeholder="Selectează un folder"
            >
                <option value="" disabled selected>Selectează un folder</option>
                @if(!$this->isRootDirectory())
                    <option value=".." selected>Folderul anterior (../)</option>
                @endif
