...
class Manager extends Component implements HasActions, HasForms
{
    ...
    public function storage($disk = false)
    {
        ...
        $storageClass = get_class(Storage::getFacadeRoot());
        ...
        if ($disk) {
            $classInstance = $classInstance->disk($disk);
        }
        ...
        foreach ($storageItems as $item) {
            if ($item['type'] == 'dir') {
                ...
                $files[] = (object) [
                    ...
                    'last_modified' => $item['timestamp'] ?? Carbon::parse($item->lastModified())->diffForHumans(),
                    'thumbnails' => [],
                ];
            }
        }
        ...
        foreach ($allFiles as $file) {
            ...
            if (strlen($search) >= $minMatchLength && stripos($fileName, $search) !== false) {
                ...
                $results[] = [
                    ...
                    'last_modified' => Carbon::parse(Storage::lastModified($file))->diffForHumans(),
