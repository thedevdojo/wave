...
<a {{ $attributes }} href="{{ $href }}" @if(($href ?? false) && $target == '_self') wire:navigate @else target="_blank" @endif class="@if($isActive){{ 'text-zinc-900 bg-zinc-100 font-medium dark:border-white dark:bg-zinc-700/60 dark:text-zinc-100' }}@endif transition-colors px-2.5 py-2 flex rounded-md w-full h-auto text-sm hover:bg-zinc-100 dark:hover:bg-zinc-700/60 justify-start items-center hover:text-zinc-900 dark:hover:text-zinc-100 space-x-2 overflow-hidden group-hover:autoflow-auto items">
