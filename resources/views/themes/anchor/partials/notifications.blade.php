...
@if(!isset($show_all_notifications))
    ...
    <div id="notification-list" @click.away="open = false" class="relative flex items-center h-full" x-data="{ open: false }">
        <div id="notification-icon relative">
            <button @click="open = !open" class="relative p-1 ml-3 text-zinc-400 transition duration-150 ease-in-out rounded-full hover:text-zinc-500 focus:outline-none focus:text-zinc-500 focus:bg-zinc-100">
    ...
    @endif
        ...
        @if(!isset($show_all_notifications))
            <div id="notification-header">
                <div id="notification-head-content" class="flex items-center w-full px-3 py-3 text-zinc-600 border-b border-zinc-200">
            ...
            <div class="relative">
                ...
                @foreach ($unreadNotifications as $index => $notification)
                    ...
                    <div id="notification-li-{{ $index + 1 }}" class="flex flex-col pb-5 border-b border-zinc-200 @if(!isset($show_all_notifications)){{ 'hover:bg-zinc-50' }}@endif">
                        ...
                        <span data-id="{{ $notification->id }}" data-listid="{{ $index+1 }}" class="flex justify-start w-full py-1 pl-16 ml-1 text-xs text-zinc-500 cursor-pointer k hover:text-zinc-700 mark-as-read hover:underline">
        ...
        @if(!isset($show_all_notifications))
            <div id="notification-footer" class="flex items-center justify-center py-3 text-xs font-medium text-zinc-600 bg-zinc-100 border-t border-zinc-200 ">
