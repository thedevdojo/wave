    ...
    @else
        @if(auth()->user()->subscriber())
            ...
            <div class="flex flex-col">
                ...
                <button data-url="{{ auth()->user()->subscription->cancel_url }}" class="inline-flex self-start justify-center w-auto px-4 py-2 mt-5 text-sm font-medium text-white transition duration-150 ease-in-out bg-red-500 border border-transparent rounded-md checkout-cancel hover:bg-red-600 focus:outline-none focus:border-red-600 focus:shadow-outline-red-500 active:bg-red-600">Cancel Subscription</button>
            </div>
        @else
