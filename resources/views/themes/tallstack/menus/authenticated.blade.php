<div x-data="{ open: false }" class="flex h-full md:flex-1">
    <div class="hidden flex-1 space-x-8 h-full md:flex">
        ...
        <div x-data="{ dropdown: false }" @mouseenter="dropdown = true" @mouseleave="dropdown=false" @click.away="dropdown=false" class="inline-flex relative items-center px-1 pt-1 text-sm font-medium leading-5 text-zinc-500 border-b-2 border-transparent transition duration-150 ease-in-out cursor-pointer hover:text-zinc-700 hover:border-zinc-300 focus:outline-none focus:text-zinc-700 focus:border-zinc-300">
            <span>Resources</span>
        ...
        <a href="#" class="inline-flex items-center px-1 pt-1 text-sm font-medium leading-5 text-zinc-500 border-b-2 border-transparent transition duration-150 ease-in-out hover:text-zinc-700 hover:border-zinc-300 focus:outline-none focus:text-zinc-700 focus:border-zinc-300">Support</a>
    </div>
    ...
    <div class="flex sm:ml-6 sm:items-center">
        ...
        <div @click.away="open = false" class="flex relative items-center ml-3 h-full" x-data="{ open: false }">
            ...
            <div
                ...
                <div class="bg-white rounded-xl border border-zinc-100 shadow-md" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                    ...
                    @impersonating
                            <a href="{{ route('impersonate.leave') }}" class="block px-4 py-2 text-sm leading-5 text-blue-900 text-zinc-700 bg-blue-50 border-t border-zinc-100 hover:bg-blue-100 focus:outline-none focus:bg-blue-200">Leave impersonation</a>
                    @endImpersonating
                    <div class="border-t border-zinc-100"></div>
                    <div class="py-1">

                        <div class="block px-4 py-1">
                            <span class="inline-block px-2 my-1 -ml-1 text-xs font-medium leading-5 text-zinc-600 bg-zinc-200 rounded">{{ \Wave\Role::find(auth()->user()->role_id)->name }}</span>
                        </div>
                        @trial
