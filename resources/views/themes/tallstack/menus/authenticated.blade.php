<div x-data="{ open: false }" class="flex h-full md:flex-1">
    <div class="hidden flex-1 space-x-8 h-full md:flex">
        ...
        <div x-data="{ dropdown: false }" @mouseenter="dropdown = true" @mouseleave="dropdown=false" @click.away="dropdown=false" class="inline-flex relative items-center px-1 pt-1 text-sm font-medium leading-5 text-gray-500 border-b-2 border-transparent transition duration-150 ease-in-out cursor-pointer hover:text-gray-700 hover:border-gray-300 focus:outline-none focus:text-gray-700 focus:border-gray-300">
            <span>Resources</span>
            ...
            <div x-show="dropdown"
                ...
                <div class="rounded-xl border border-gray-100 shadow-md">
                    <div class="overflow-hidden rounded-xl shadow-xs">
                        <div class="grid relative z-20 gap-6 px-5 py-6 bg-white sm:p-8 sm:gap-8">
                            <a href="{{ url('docs') }}" class="block px-5 py-3 -m-3 space-y-1 rounded-xl transition duration-150 ease-in-out hover:border-blue-500 hover:border-l-2 hover:bg-gray-100">
                            ...
                            <a href="https://devdojo.com/course/wave" target="_blank" class="block px-5 py-3 -m-3 space-y-1 rounded-xl transition duration-150 ease-in-out hover:bg-gray-100">
                                <p class="text-base font-medium leading-6 text-gray-900">
                                    Videos
                                </p>
                                <p class="text-xs leading-5 text-gray-500">
                            ...
                            <a href="{{ route('wave.blog') }}" class="block px-5 py-3 -m-3 space-y-1 rounded-xl transition duration-150 ease-in-out hover:bg-gray-100">
                                <p class="text-base font-medium leading-6 text-gray-900">
                                    From The Blog
                                </p>
                                <p class="text-xs leading-5 text-gray-500">
        ...
        <a href="#" class="inline-flex items-center px-1 pt-1 text-sm font-medium leading-5 text-gray-500 border-b-2 border-transparent transition duration-150 ease-in-out hover:text-gray-700 hover:border-gray-300 focus:outline-none focus:text-gray-700 focus:border-gray-300">Support</a>
    </div>
    ...
    <div class="flex sm:ml-6 sm:items-center">
        ...
        @if( auth()->user()->onTrial() )
            <div class="hidden relative justify-center items-center h-full md:flex">
                <span class="px-3 py-1 text-xs text-red-600 bg-red-100 rounded-md border border-gray-200">You have {{ auth()->user()->daysLeftOnTrial() }} @if(auth()->user()->daysLeftOnTrial() > 1){{ 'Days' }}@else{{ 'Day' }}@endif left on your Trial</span>
            </div>
        @endif
        ...
        <div @click.away="open = false" class="flex relative items-center ml-3 h-full" x-data="{ open: false }">
            <div>
                <button @click="open = !open" class="flex text-sm rounded-full border-2 border-transparent transition duration-150 ease-in-out focus:outline-none focus:border-gray-300" id="user-menu" aria-label="User menu" aria-haspopup="true" x-bind:aria-expanded="open" aria-expanded="true">
            ...
            <div
                ...
                <div class="bg-white rounded-xl border border-gray-100 shadow-md" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                    ...
                    @impersonating
                            <a href="{{ route('impersonate.leave') }}" class="block px-4 py-2 text-sm leading-5 text-blue-900 text-gray-700 bg-blue-50 border-t border-gray-100 hover:bg-blue-100 focus:outline-none focus:bg-blue-200">Leave impersonation</a>
                    @endImpersonating
                    <div class="border-t border-gray-100"></div>
                    <div class="py-1">

                        <div class="block px-4 py-1">
                            <span class="inline-block px-2 my-1 -ml-1 text-xs font-medium leading-5 text-gray-600 bg-gray-200 rounded">{{ \Wave\Role::find(auth()->user()->role_id)->name }}</span>
                        </div>
                        @trial
                    ...
                    <div class="border-t border-gray-100"></div>
                    <div class="py-1">
                        <a href="{{ route('logout') }}" class="block px-4 py-2 w-full text-sm leading-5 text-left text-gray-700 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:bg-gray-100 focus:text-gray-900" role="menuitem">
                            Sign out
                        </a>
                    </div>
                </div>
