<section class="block w-full bg-white border-b border-gray-200 dark:border-gray-800 dark:bg-black" x-data="{ mobileNavOpen: false, menuItem: '' }" x-init="menuItem = window.location.pathname.replace(/^\/|\/$/g, '')">
    <x-app.container>
        <nav class="pt-3.5 pb-3.5 md:pb-0">
            <div class="flex justify-between items-center md:mb-2">
                ...
                <div class="px-2 mr-auto w-full md:w-auto">
                    <div class="flex items-center pr-4 text-gray-400 rounded-lg border border-gray-200 ring-1 ring-transparent focus-within:ring-black dark:bg-gray-800 dark:border-gray-900 focus-within:text-gray-900 dark:focus-within:text-gray-100 focus-within:border-gray-800">
                        <x-phosphor-magnifying-glass class="ml-2.5 w-4 h-4 translate-x-px fill-current" />
                        <input class="px-2 py-2 w-full text-sm placeholder-gray-400 text-gray-800 bg-transparent border-0 outline-none md:w-auto peer focus:outline-none focus:border-0 focus:ring-0 dark:text-gray-100" type="text" placeholder="Type to search">
                    </div>
                </div>
                
                <div class="hidden pl-2 w-full md:block md:w-auto">
                        ...
                        <div class="flex items-center mb-6 w-full text-[13px] text-gray-500 md:w-auto md:mb-0">
                            ...
                            <a href="{{ route('notifications') }}" class="relative mr-4 cursor-pointer">
                                ...
                                @if(auth()->user()->notifications()->count())
                                    <span class="absolute top-0 right-0 w-3 h-3 bg-red-600 rounded-full border-2 border-white transform translate-x-1/4 -translate-y-1"></span>
                                @endif
                            </a>
            ...
            <div 
                ...
                x-init="
                    ...
                    })" class="hidden relative pt-2 md:flex md:flex-wrap">
                <div x-ref="marker" :class="{ 'opacity-0' : !hovering, 'opacity-100' : hovering }" class="absolute transition-opacity left-0 rounded-lg bg-black/[7%] dark:bg-white/[15%]" x-cloak></div>
    ...
    <div :class="{'block': mobileNavOpen, 'hidden': !mobileNavOpen}" class="hidden fixed top-0 bottom-0 left-0 z-50">
        ...
        <nav class="fixed z-20 w-full h-full bg-white dark:bg-gray-900 max-w-xss">
            <div class="flex flex-wrap justify-between items-center py-3.5 border-b border-gray-200 dark:border-gray-800">
                <x-app.container class="flex justify-between items-center w-full">
                    <div class="px-1 w-auto">
                        <a class="inline-block h-full translate-y-[3px]" href="{{ route('home') }}">
                            <x-logo-icon class="w-auto h-8 text-black dark:text-white"></x-logo-icon>
                        </a>
                    </div>
                    <div x-data="{ url: window.location.pathname }" class="px-2.5 w-full">
            ...
            <div class="overflow-y-auto py-5 h-full">
                <x-app.container class="flex flex-col justify-between items-center space-y-1 w-full text-gray-600 dark:text-gray-200">

                        <a x-on:click="mobileNavOpen = false;" href="https://devdojo.com/wave/docs" target="_blank" class="flex items-center px-3 py-2 w-full rounded-md hover:bg-black/5 dark:hover:bg-white/10" href="#">
                            <x-phosphor-notebook-duotone class="mr-3 w-5 h-5" />
                            <span class="font-medium">Documentation</span>
                        </a>
                        <a x-on:click="mobileNavOpen = false" href="/dashboard/support" class="flex items-center px-3 py-2 w-full rounded-md hover:bg-black/5 dark:hover:bg-white/10" href="#">
                            <x-phosphor-question-duotone class="mr-3 w-5 h-5" />
                            <span class="font-medium">Help</span>
                        </a>
                        <a 
                            href="{{ route('changelogs') }}"
                            x-on:click="mobileNavOpen = false" class="flex items-center px-3 py-2 w-full rounded-md hover:bg-black/5 dark:hover:bg-white/10">
                            <x-phosphor-megaphone-duotone class="mr-3 w-5 h-5" />
                            <span class="font-medium">Changelog</span>
                        </a>
                        <a x-on:click="mobileNavOpen = false; demoButtonClickMessage(event)" class="flex items-center px-3 py-2 w-full rounded-md hover:bg-black/5 dark:hover:bg-white/10" href="#">
                            <x-phosphor-chat-text-duotone class="mr-3 w-5 h-5" />
