<div x-data="{ sidebarOpen: false }"  @open-sidebar.window="sidebarOpen = true"
    ...
    {{-- Backdrop for mobile --}}
    <div x-show="sidebarOpen" @click="sidebarOpen=false" class="fixed top-0 end-0 z-50 w-screen h-screen duration-300 ease-out bg-black/20 dark:bg-white/10"></div>
    
    {{-- Sidebar --}} 
    <div :class="{ 'ltr:-translate-x-full rtl:translate-x-full': !sidebarOpen }"
        class="fixed top-0 start-0 flex items-stretch ltr:-translate-x-full rtl:translate-x-full overflow-hidden lg:ltr:translate-x-0 lg:rtl:translate-x-0 z-50 h-dvh md:h-screen transition-[width,transform] duration-150 ease-out bg-zinc-50 dark:bg-zinc-900 w-64 group @if(config('wave.dev_bar')){{ 'pb-10' }}@endif">  
        <div class="flex flex-col justify-between w-full overflow-auto md:h-full h-svh pt-4 pb-2.5">
            <div class="relative flex flex-col">
                <button x-on:click="sidebarOpen=false" class="flex items-center justify-center flex-shrink-0 w-10 h-10 ms-4 rounded-md lg:hidden text-zinc-400 hover:text-zinc-800 dark:hover:text-zinc-200 dark:hover:bg-zinc-700/70 hover:bg-gray-200/70">
                ...
                <div class="flex items-center px-5 space-x-2 rtl:space-x-reverse">
                    <a href="/" class="flex justify-center items-center py-4 ps-0.5 space-x-1 rtl:space-x-reverse font-bold text-zinc-900">
            ...
            <div class="relative px-2.5 space-y-1.5 text-zinc-700 dark:text-zinc-400">
                ...
                <div x-show="sidebarTip" x-data="{ sidebarTip: $persist(true) }" class="px-1 py-3" x-collapse x-cloak>
                    <div class="relative w-full px-4 py-3 space-y-1 border rounded-lg bg-zinc-50 text-zinc-700 dark:text-zinc-100 dark:bg-zinc-800 border-zinc-200/60 dark:border-zinc-700">
                        <button @click="sidebarTip=false" class="absolute top-0 end-0 z-50 p-1.5 mt-2.5 me-2.5 rounded-full opacity-80 cursor-pointer hover:opacity-100 hover:bg-zinc-100 hover:dark:bg-zinc-700 hover:dark:text-zinc-300 text-zinc-500 dark:text-zinc-400">
