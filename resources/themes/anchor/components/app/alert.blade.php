...
<div 
    x-data="{
        ...
        init() {
            ...
            let stored = localStorage.getItem('alert_{{ $id }}');
            if (stored !== null) {
                this.show = stored === 'true';
            }

            // Watch for changes and persist to localStorage
            this.$watch('show', (val) => {
                localStorage.setItem('alert_{{ $id }}', val);
            });
        }
    }"
    x-show="show"
...
>
    @if($dismissable)
        <button 
            @click="show = false" 
            class="absolute right-0 top-0 z-50 p-1.5 mr-3 rounded-full opacity-70 mt-3.5 cursor-pointer hover:opacity-100 hover:bg-zinc-200 hover:dark:bg-zinc-700 hover:dark:text-zinc-300 text-zinc-500 dark:text-zinc-400"
        >
