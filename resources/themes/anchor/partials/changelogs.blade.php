...
<div id="changeLogNotification" class="flex fixed right-0 bottom-0 z-50 justify-center items-end px-4 py-6 opacity-100 transition-all duration-200 ease-out transform translate-y-0 cursor-pointer pointer-events-none hover:-translate-y-1 sm:p-6 sm:items-start sm:justify-end" data-href="{{ route('changelog', ['changelog' => $changeLogNotification->id]) }}">
	<div id="changelog_content" class="relative w-full max-w-sm bg-white rounded-lg border shadow-lg pointer-events-auto dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800">
		<div class="relative p-5">
			...
			<h4 class="overflow-hidden pr-7 w-full text-sm font-medium leading-5 whitespace-nowrap text-zinc-900 dark:text-white text-ellipsis">{{ $changeLogNotification->title }}</h4>
			<p class="mt-2 text-sm tracking-wide leading-5 text-zinc-500">{{ $changeLogNotification->description }}</p>
			<div id="changelog_footer" class="mt-1"><a href="{{ route('changelog', ['changelog' => $changeLogNotification->id]) }}" class="text-sm font-medium leading-5 opacity-90 transition duration-150 ease-in-out hover:opacity-100 focus:outline-none focus:underline" style="color:{{ config('wave.primary_color') }}">Learn More</a></div>
		</div>
	</div>
</div>
