            ...
            <a href="{{ @$notification_data->link }}" class="flex items-start p-5 pb-2">
                ...
                <div class="flex flex-col flex-1 items-start ml-3 w-0">
                    <p class="text-sm leading-5 text-zinc-600">
                        <strong>{{ @$notification_data->user['username'] }} @if(isset($notification_data->type) && @$notification_data->type == 'message'){{ 'a lăsat un mesaj' }}@else{{ 'a spus' }}@endif</strong>
                        {{ @$notification_data->body }} în <span class="notification-highlight">{{ @$notification_data->title }}</span>
                    </p>
