<div x-data="{ open: false }" class="flex h-full md:flex-1">
    <div class="hidden flex-1 space-x-8 rtl:space-x-reverse h-full font-semibold md:flex">
        ...
        <div x-data="{ dropdown: false }" @mouseenter="dropdown = true" @mouseleave="dropdown=false" @click.away="dropdown=false" class="inline-flex relative items-center px-1 pt-1 text-sm leading-5 border-b-2 border-transparent transition duration-150 ease-in-out cursor-pointer text-zinc-800 hover:text-zinc-900 hover:border-zinc-300">
            <span>Resources</span>
            <svg class="ms-1 w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
            ...
            <div x-show="dropdown"
                ...
                class="absolute top-0 start-1/2 px-2 mt-20 w-screen max-w-xs transform ltr:-translate-x-1/2 rtl:translate-x-1/2 sm:px-0" x-cloak>
    ...
    <div class="flex sm:ms-6 sm:items-center">
        ...
        <!-- Profile dropdown -->
        <div @click.away="open = false" class="flex relative items-center ms-3 h-full" x-data="{ open: false }">
            <div>
            ...
            <div
                ...
                x-transition:leave-start="opacity-100 scale-100"
                x-transition:leave-end="opacity-0 scale-95"
                class="absolute top-0 end-0 mt-20 w-56 rounded-xl transform origin-top-right" x-cloak>
                ...
                <div class="bg-white rounded-xl border shadow-md border-zinc-100" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                    ...
                    <div class="py-1">
                        ...
                        <div class="block px-4 py-1">
                            <span class="inline-block px-2 my-1 -ms-1 text-xs font-medium leading-5 rounded text-zinc-600 bg-zinc-200">{{ auth()->user()->roles->first()->name }}</span>
                        </div>
                        @trial
                    ...
                    <div class="border-t border-zinc-100"></div>
                    <div class="py-1">
                        <a href="{{ route('logout') }}" class="block px-4 py-2 w-full text-sm leading-5 text-start text-zinc-700 hover:bg-zinc-100 hover:text-zinc-900 focus:outline-none focus:bg-zinc-100 focus:text-zinc-900" role="menuitem">
                            Sign out
                        </a>
                    </div>
                </div>
