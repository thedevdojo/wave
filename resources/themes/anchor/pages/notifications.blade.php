...
class extends Component
{
    ...
    {
        $this->unreadNotifications = auth()->user()->unreadNotifications->all();
                        ...
                        <a href="{{ @$notification_data->link }}" wire:navigate class="flex items-start p-5 pb-2">
                            ...
                            <div class="flex flex-col flex-1 items-start ml-3 space-y-1 w-0">
                                <div class="flex relative items-center space-x-2">
                                    ...
                                    <time class="text-xs font-medium leading-5 text-zinc-500">{{ \Carbon\Carbon::parse(@$notification->created_at)->format('F, jS h:i A') }}</time>
                                </div>
