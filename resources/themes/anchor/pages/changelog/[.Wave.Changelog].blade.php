    ...
    <x-app.container>
        <x-card class="lg:p-10">
            ...
            <article id="changelog-{{ $changelog->id }}" class="max-w-4xl mx-auto mt-5">
                ...
                <p class="mt-5 text-xs font-medium tracking-wider text-zinc-800">Postat la <time datetime="{{ Carbon\Carbon::parse($changelog->created_at)->toIso8601String() }}" class="ml-1">{{ Carbon\Carbon::parse($changelog->created_at)->toFormattedDateString() }}</time></p>
